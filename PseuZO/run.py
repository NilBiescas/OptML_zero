...
class Framework:
    ...
    def __init__(self, args, task):
        ...
        if args.logits == "False":
            pass
            #self.attach_last_hidden_state_hook()
    def attach_last_hidden_state_hook(self):
        """
        给模型最后一层 block 注册 forward hook,
        在 model(**inputs) 运行后，可以从 model._last_hidden_state 读取。
        """
        self.model._last_hidden_state = None  # 缓存区

        def hook_fn(module, input, output):
            # output 就是该层的输出 (hidden states)
            # 保存最后一层的 hidden state
            self.model._last_hidden_state = output[0].clone().detach() if isinstance(output, tuple) else output.clone().detach()

        # 找到最后一个 transformer block
        if hasattr(self.model, "model"):  # OPT, LLaMA, etc.
            if hasattr(self.model.model, "layers"):  
                last_block = self.model.model.layers[-1]
            elif hasattr(self.model.model, "decoder"):  # Bart-like
                last_block = self.model.model.decoder.layers[-1]
            else:
                raise ValueError("未知的 transformer 结构")
        else:
            raise ValueError("模型不符合预期结构")
    ...
    def train(self, train_samples, eval_samples):
        ...
        if self.args.only_train_option and not self.args.non_diff:
            ...
            elif self.args.logits == "False":
                self.model.forward = forward_wrap_with_option_len_pzo.__get__(self.model,type(self.model))
            else:
                self.model.forward = forward_wrap_with_option_len_pzo_logits.__get__(self.model,type(self.model))
        ...
        if trainer.is_world_process_zero():
            ...
            if hf_token:
                try:
                    ...
                    repo_name = f"{username}/pseuzo-best-{self.args.task_name}-{model_clean_name}"
