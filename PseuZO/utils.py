...
def forward_wrap_with_option_len(self, need_grad=False,Hessian_estimate=False,input_ids=None, labels=None, option_len=None, num_options=None, return_dict=None, **kwargs):
    """
    ...
    if num_options is not None: 
        ...
        if any([x != num_options[0] for x in num_options]):
            ...
            #selected_log_probs_clone = selected_log_probs.detach().clone().requires_grad_(True)
...
def forward_wrap_with_option_len_pzo(self, need_grad=False,Hessian_estimate=False,adjust_lm_head=False,input_ids=None, labels=None, option_len=None, num_options=None, return_dict=None, **kwargs):
    """
    This is to replace the original forward function of Transformer models to enable:
    (1) Partial target sequence: loss will only be calculated on part of the sequence
    (2) Classification-style training: a classification loss (CE) will be calculated over several options
    ...
    with torch.enable_grad():
        ...
        if num_options is not None: 
            # Train as a classification tasks
            # Shift so that tokens < n predict n
            #shift_logits = self.lm_head(last_hidden_state)[..., :-1, :].contiguous()
            log_probs = F.log_softmax(self.lm_head(last_hidden_state)[..., :-1, :].contiguous(), dim=-1)
            mask = shift_labels != -100 # Option part
            shift_labels[~mask] = 0 # So that it doesn't mess up with indexing
    ...
    if not need_grad:
        grad_last = None
        loss = (loss.detach(),last_hidden_state,grad_last)
    else:
        #loss.backward(retain_graph=False, create_graph=Hessian_estimate)
        if adjust_lm_head:
            grad_last = torch.autograd.grad(loss,[last_hidden_state,self.lm_head.weight],create_graph=Hessian_estimate)
        else:
            grad_last = torch.autograd.grad(loss,last_hidden_state,create_graph=Hessian_estimate)
        #grad_last = last_hidden_state.grad.data
        loss = (loss.detach(),last_hidden_state,grad_last)
...
def forward_wrap_with_option_len_pzo_logits(self, need_grad=False,Hessian_estimate=False,adjust_lm_head=False,input_ids=None, labels=None, option_len=None, num_options=None, return_dict=None, **kwargs):
    """
    Here we only compute the gradients for logits, not the last hidden state.
    As for Llama series, the untie-embedding with lm_head can be a problem which OPT/GPT series actually do not encounter.
    """
    ...
    if not need_grad:
        grad_last = None
        loss = (loss.detach(),logits,grad_last)
    else:
        #loss.backward(retain_graph=False, create_graph=Hessian_estimate)
        if adjust_lm_head:
            grad_last = torch.autograd.grad(loss,[logits,self.lm_head.weight],create_graph=Hessian_estimate)
        else:
            grad_last = torch.autograd.grad(loss,logits,create_graph=Hessian_estimate)
        #grad_last = last_hidden_state.grad.data
        loss = (loss.detach(),logits,grad_last)
