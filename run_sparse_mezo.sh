#!/bin/bash
# Runs INSIDE the pod to execute Sparse-MeZO training.
# Called by submit_sparse_mezo.sh via runai submit --command.

set -euo pipefail

mkdir -p runs/sparse_mezo hf_cache wandb
...
echo ">>> Starting Sparse-MeZO training (${CONFIG_FILE})"
accelerate launch --num_processes 4 train.py --config "${CONFIG_FILE}"

echo ">>> Sparse-MeZO training complete."
