...
for DATASET in SST2 RTE; do
  ...
  runai submit \
    ...
    --command -- bash -c "ln -sf /usr/bin/python3 /usr/bin/python && git clone -b nil_branch https://\${GITHUB_TOKEN}@github.com/NilBiescas/OptML_zero.git && cd OptML_zero/PseuZO && pip install -r requirements.txt && TRAIN=10 DEV=10 EVAL=10 CUDA_VISIBLE_DEVICES=0 TRAINER=pzo MODEL=facebook/opt-1.3b TASK=${DATASET} MODE=ft LR=1e-7 EPS=1e-3 BS=16 STEPS=10 EVAL_STEPS=5 bash mezo.sh"
