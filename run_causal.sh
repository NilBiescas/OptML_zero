#!/bin/bash
# Runs INSIDE the pod to execute generative causal language modeling LOZO training.
# Called by submit_causal.sh via runai submit --command.

set -euo pipefail

# Create local directories for cache and logs in current directory
mkdir -p runs/causal hf_cache wandb
...
echo ">>> Installing dependencies for Causal LOZO"
pip install --upgrade transformers datasets accelerate evaluate pyyaml

echo ">>> Starting Causal LOZO training (config_causal.yaml)"
accelerate launch --num_processes 4 train_causal.py --config config_causal.yaml

echo ">>> Causal LOZO Training complete."
